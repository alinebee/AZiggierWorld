...
test "Report sizes for each game part" {
    ...
    for (GamePart.Enum.all) |part| {
        ...
        const bytecode = try resource_directory.resourceDescriptor(resource_ids.bytecode);
        ...
        const palettes = try resource_directory.resourceDescriptor(resource_ids.palettes);
        ...
        const polygons = try resource_directory.resourceDescriptor(resource_ids.polygons);
        ...
        if (resource_ids.animations) |animation_id| {
            const animations = try resource_directory.resourceDescriptor(animation_id);
