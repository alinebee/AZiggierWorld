...
pub const RegisterShiftRight = struct {
    ...
    pub const opcode = Opcode.RegisterShiftRight;
...
test "parse parses valid bytecode and consumes 4 bytes" {
    const instruction = try expectParse(RegisterShiftRight.parse, &RegisterShiftRight.Fixtures.valid, 4);
...
test "parse returns error.ShiftTooLarge and consumes 4 bytes on invalid shift distance" {
    try testing.expectError(
        error.ShiftTooLarge,
        expectParse(RegisterShiftRight.parse, &RegisterShiftRight.Fixtures.invalid_shift, 4),
    );
}
...
test "execute shifts destination register" {
    ...
    const instruction = RegisterShiftRight{
