...
pub fn parse(program: *Program.Instance) Error!Wrapped {
...
fn wrap(comptime field_name: []const u8, comptime parseFn: anytype, raw_opcode: Opcode.Raw, program: *Program.Instance) Error!Wrapped {
    return @unionInit(Wrapped, field_name, try parseFn(raw_opcode, program));
}
